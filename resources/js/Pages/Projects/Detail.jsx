...
export default function ProjectDetail({ slug = 'monitoring-dapur-mbg', dbProject = null, recentCommits = [] }) {
    ...
    const project = dbProject
        ? {
              ...
              subtitle: dbProject.description || 'Proyek tercatat dalam WorkTrack.',
              category: dbProject.category || 'Web Application',
              ...
              overview: dbProject.description || 'Proyek ini dikembangkan dan dicatat menggunakan sistem WorkTrack.',
              techStack: (dbProject.tech_stack || []).map((t) => ({ name: t, icon: 'code' })),
              keyFeatures: [
                  'Manajemen data dan fitur utama sistem',
                  'Pencatatan aktivitas dan status pengerjaan',
                  'Desain responsif dan antarmuka modern',
                  'Integrasi alur kerja terstruktur',
              ],
              challenges: [
                  'Optimalisasi performa dan struktur kode',
                  'Penyusunan alur kerja yang intuitif dan mudah dipelihara',
              ],
              contributions: [
                  'Mengembangkan antarmuka dan logika sistem',
                  'Menghubungkan integrasi dan konfigurasi teknis',
              ],
              results: [
    ...
    return (
        <div className={`min-h-screen font-sans antialiased transition-colors ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-white text-slate-800'}`}>
            ...
            <main className="max-w-[1580px] mx-auto px-4 sm:px-6 lg:px-8 pt-10 sm:pt-14 relative z-20 pb-20 space-y-16">
                ...
                {/* ========================================================== */}
                {/* 4.5 RECENT COMMITS & ACTIVITY (from GitHub)                */}
                {/* ========================================================== */}
                {(project.githubDisplay || (recentCommits && recentCommits.length > 0)) && (
                    <div className={`p-6 sm:p-8 rounded-xl border shadow-xs space-y-6 ${isDarkMode ? 'bg-slate-900/70 border-slate-800' : 'bg-white border-slate-200'}`}>
                        <div className="flex flex-wrap items-center justify-between gap-3 border-b pb-4 border-slate-100 dark:border-slate-800">
                            <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-lg bg-slate-900 text-white dark:bg-slate-800 dark:text-blue-400 flex items-center justify-center shrink-0">
                                    <GithubIcon className="w-4 h-4" />
                                </div>
                                <div>
                            ...
                            {project.githubUrl && (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-200 transition-colors"
                                >
                                    <span>Riwayat Commit Lengkap</span>
                        ...
                        {recentCommits && recentCommits.length > 0 ? (
                                ...
                                {recentCommits.map((c, idx) => (
                                    <div key={idx} className="relative group">
                                        ...
                                        <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-2">
                                            <span>Oleh <strong className="text-slate-700 dark:text-slate-300">{c.author}</strong></span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="py-6 text-center text-xs text-slate-400 dark:text-slate-500 italic">
                                Belum ada riwayat commit yang dapat ditampilkan untuk repository ini atau repository masih kosong.
                            </div>
                        )}
                    </div>
                )}
